...
@Component({
  selector: 'app-account-order-status-cancel',
  templateUrl: './account-order-status-cancel.component.html',
  styleUrl: './account-order-status-cancel.component.scss'
})
export class AccountOrderStatusCancelComponent implements OnInit{
  ...
  getOrder(){
    this.callApiService.getOrderStatus('CANCEL', this.currentPage).subscribe({
      next:(data)=>{
        console.log(data)
        if(data['error'] === true){

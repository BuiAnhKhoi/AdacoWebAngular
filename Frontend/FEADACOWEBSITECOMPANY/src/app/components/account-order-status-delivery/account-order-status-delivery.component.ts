...
@Component({
  selector: 'app-account-order-status-delivery',
  templateUrl: './account-order-status-delivery.component.html',
  styleUrl: './account-order-status-delivery.component.scss'
})
export class AccountOrderStatusDeliveryComponent implements OnInit{
  ...
  getOrder(){
    this.callApiService.getOrderStatus('DELIVERY', this.currentPage).subscribe({
      next:(data)=>{
        console.log(data)
        if(data['error'] === true){

...
@Component({
  selector: 'app-account-order-status-need-delivery',
  templateUrl: './account-order-status-need-delivery.component.html',
  styleUrl: './account-order-status-need-delivery.component.scss'
})
export class AccountOrderStatusNeedDeliveryComponent implements OnInit{
  ...
  getOrder(){
    this.callApiService.getOrderStatus('NEED-DELIVERY', this.currentPage).subscribe({
      next:(data)=>{
        console.log(data)
        if(data['error'] === true){

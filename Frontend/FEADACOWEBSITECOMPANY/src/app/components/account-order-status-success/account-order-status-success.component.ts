...
@Component({
  selector: 'app-account-order-status-success',
  templateUrl: './account-order-status-success.component.html',
  styleUrl: './account-order-status-success.component.scss'
})
export class AccountOrderStatusSuccessComponent implements OnInit{
  ...
  getOrder(){
    this.callApiService.getOrderStatus('SUCCESS', this.currentPage).subscribe({
      next:(data)=>{
        console.log(data)
        if(data['error'] === true){
